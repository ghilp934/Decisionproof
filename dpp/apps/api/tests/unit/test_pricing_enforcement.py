...
class TestRPMEnforcement:
    ...
    def test_rpm_within_limit_allows_request(self, mock_ssot_with_tiers, mock_redis):
        """Request within RPM limit should be allowed."""
    ...
    def test_rpm_exceeds_limit_blocks_request(self, mock_ssot_with_tiers, mock_redis):
        """Request exceeding RPM limit should be blocked."""
        engine = EnforcementEngine(mock_ssot_with_tiers, mock_redis)
        tier = mock_ssot_with_tiers.tiers[0]
        ...
        assert result["current_count"] == 601
        assert result["limit"] == 600
        assert result["violated_policy"] == "rpm"

    def test_rpm_zero_means_unlimited(self, mock_ssot_with_tiers, mock_redis):
        """RPM = 0 should mean unlimited (no enforcement)."""
        engine = EnforcementEngine(mock_ssot_with_tiers, mock_redis)
...
class TestMonthlyQuotaEnforcement:
    ...
    def test_quota_within_limit_allows_usage(self, mock_ssot_with_tiers, mock_redis):
        """Usage within monthly quota should be allowed."""
        engine = EnforcementEngine(mock_ssot_with_tiers, mock_redis)
        tier = mock_ssot_with_tiers.tiers[0]
        ...
        result = engine.check_monthly_dc_quota("ws_123", tier, dc_amount=100)

        assert result["allowed"] is True
        assert result["current_usage"] == 1500
        assert result["limit"] == 2000
        assert result["projected_usage"] == 1600

    def test_quota_exceeds_limit_blocks_usage(self, mock_ssot_with_tiers, mock_redis):
        """Usage exceeding monthly quota should be blocked."""
        engine = EnforcementEngine(mock_ssot_with_tiers, mock_redis)
        tier = mock_ssot_with_tiers.tiers[0]
        ...
        assert result["allowed"] is False
        assert result["current_usage"] == 1950
        assert result["limit"] == 2000
        assert result["projected_usage"] == 2050
        assert result["violated_policy"] == "monthly_dc"

    def test_quota_zero_means_unlimited(self, mock_ssot_with_tiers, mock_redis):
        """Monthly quota = 0 should mean unlimited (no enforcement)."""
        engine = EnforcementEngine(mock_ssot_with_tiers, mock_redis)
...
class TestGraceOverageEnforcement:
    ...
    def test_hard_cap_within_limit_allows_usage(self, mock_ssot_with_tiers, mock_redis):
        """Usage within hard cap should be allowed."""
        engine = EnforcementEngine(mock_ssot_with_tiers, mock_redis)
        tier = mock_ssot_with_tiers.tiers[0]

        # Mock: current overage is 500 DC (below 1000 hard cap)
        mock_redis.get.return_value = "500"

        result = engine.check_hard_overage_cap("ws_123", tier, dc_amount=100)

        assert result["allowed"] is True
        assert result["current_overage"] == 500
        assert result["hard_cap"] == 1000
        assert result["projected_overage"] == 600
    ...
    def test_hard_cap_exceeds_limit_applies_grace(self, mock_ssot_with_tiers, mock_redis):
        ...
        # Mock: current overage is 950 DC, adding 100 DC would be 1050 DC
        # Hard cap = 1000 DC
        # Grace = min(1% of 1000, 100 DC) = min(10, 100) = 10 DC
        # Effective cap = 1000 + 10 = 1010 DC
        # 1050 DC > 1010 DC → blocked
        mock_redis.get.return_value = "950"

        result = engine.check_hard_overage_cap("ws_123", tier, dc_amount=100)

        assert result["allowed"] is False
        assert result["current_overage"] == 950
        assert result["hard_cap"] == 1000
        assert result["projected_overage"] == 1050
        assert result["grace_overage"] == 10  # min(1%, 100 DC)
        assert result["effective_cap"] == 1010
        assert result["violated_policy"] == "hard_overage_cap"
    ...
    def test_hard_cap_zero_means_unlimited(self, mock_ssot_with_tiers, mock_redis):
        """Hard overage cap = 0 should mean unlimited (no enforcement)."""
        engine = EnforcementEngine(mock_ssot_with_tiers, mock_redis)
    ...
    def test_grace_overage_within_grace_allows_usage(self, mock_ssot_with_tiers, mock_redis):
        """Usage within grace overage should be allowed."""
        engine = EnforcementEngine(mock_ssot_with_tiers, mock_redis)
        tier = mock_ssot_with_tiers.tiers[0]

        # Mock: current overage is 1005 DC
        # Hard cap = 1000 DC
        # Grace = 10 DC
        # Effective cap = 1010 DC
        # 1005 DC < 1010 DC → allowed
        mock_redis.get.return_value = "1005"

        result = engine.check_hard_overage_cap("ws_123", tier, dc_amount=0)

        assert result["allowed"] is True
        assert result["current_overage"] == 1005
        assert result["effective_cap"] == 1010
